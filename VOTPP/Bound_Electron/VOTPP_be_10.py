...
timespace = np.linspace(0, 2.5e-4, 2001)
...
Mx_be = []
np.random.seed(8800)
seeds = list(np.random.randint(low=1,high=99999,size=22))
for seed in seeds:
...
for j in range(len(timespace)):
    ...
    for k in range(len(seeds)):
        Mx_sum += Mx_be[k][j]
    Mx_av_be.append(Mx_sum/len(seeds))

np.savetxt('both_electron_concentrations10.dat', np.column_stack((timespace,Mx_av_be)))