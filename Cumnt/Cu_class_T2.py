...
class BathSetup:
    ...
    def populate_cell(self, N, x, y, z, bath_type):
        ...
        if bath_type=='electronic':
            ...
            # Defining a rotation matrix for all coordinates (makes sure Cu(mnt) is aligned with yz-plane
            R = np.array([[-0.5301083635,0.7452628755,-0.4044358656],[-0.7452628755,-0.1820102986,0.6414479696],[0.4044358656,0.6414479696,0.651901935 ]])
            # Rotating the coordinates
            for i in range(len(self.atoms.xyz)):
                self.atoms.xyz[i] = R @ self.atoms.xyz[i]
            # Set spin type
            self.spin_types = [
